...
\begin{answer}[Page 111. \#12 (actually, this is problem \#9)]
...
\begin{answer}[\color{red} Page 111. \#12]
Apologies for doing the wrong question.

\begin{subanswer}
First note that any function of the form $f:\mathbb{R} \to \mathbb{R}$ with $f(x) = \frac{cx}{c^2 + x^2}$ for $c \neq 0$ is continuous since it is at no point undefined and the limits agree at all points.

Then in order to show continuity in each variable, we first focus on showing continuity in $x$.

Using the fact from above, we know it to be continuous on $x$ for all values of $y \neq 0$. Then for $y = 0$, consider the function $f: \mathbb{R} \to \mathbb{R}$ given by:
\begin{align*}
f(x) &= F(x \times 0) \\
&=
\begin{cases}
...
\end{cases}
\end{align*}
Then note that for any sequence ${x_n} \to 0$, $\frac{1}{x_n} \to 0$ also. Therefore $f$ is a continuous function.

The above implies that $F(x \times y)$ is continuous in $x$ and, by symmetry, is also continuous in $y$ (exchange the roles of $y$ and $x$ in the above explanation).
\end{subanswer}
\begin{subanswer}
The function $g: \mathbb{R} \to \mathbb{R}$ is given by:
\begin{align*}
g(x) &= F(x \times x) \\
&= \begin{cases}
\frac{1}{2} & x \neq 0 \\
...
\begin{subanswer}
$F(x \times y)$ is not continuous because we can take the closed set $C =\{\frac{1}{2}\}$ and note that $F^{-1}(C) = \{(x,x) \mid x \neq 0\}$ which is not closed in $\mathbb{R}^2$ because it does not contain the limit points $(0,0)$.
\end{subanswer}
\end{answer}
